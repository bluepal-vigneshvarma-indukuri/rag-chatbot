...
async def upload_document_sync(
    ...
    """Accept a file upload, store it, run ingest synchronously, and return the final status."""
    # Validate extension
    ext = file.filename.rsplit(".", 1)[-1].lower() if "." in file.filename else ""
    if ext not in ALLOWED_EXTENSIONS:
        raise HTTPException(
    ...
    # Run ingest synchronously by awaiting it directly
    await _ingest(
        document_id, file.filename, content,
        embed_base_url, embed_model, embed_api_key, disabled_bool,
    )

    # Fetch and return the final document status
    conn = psycopg2.connect(settings.database_url)
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute(
            ...
            doc = cur.fetchone()
        return dict(doc) if doc else {"id": document_id, "status": "unknown"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to fetch final status: {e}")
    finally:
        conn.close()
